...
class TopocordpolymerTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.filelog = "./test01/test01_replicate_polymer.log"
    ...
    def test_01_simulation_box(self):
        ...
        trj_single, _ = replicate_polymer.replicate_pdb(f1, [1, 1, 1])
        ...
        trj_single, _ = replicate_polymer.replicate_pdb(f1, [1, 1, 1], boxlength=[10.1, 10.2, 10.3])
        ...
        trj_single, _ = replicate_polymer.replicate_pdb(f1, [1, 1, 1], boxangles=[88.0, 78.9, 90.0])
        ...
        trj_single, _ = replicate_polymer.replicate_pdb(f2, [1, 1, 1])
        ...
        trj_single, _ = replicate_polymer.replicate_pdb(f2, [1, 1, 1], boxlength=[10.1, 10.2, 10.3])
        bl = np.array([10.1, 10.2, 10.3], dtype=np.float32)
        al = np.array([85.06, 83.72, 75.10], dtype=np.float32)
        np.testing.assert_almost_equal(trj_single.unitcell_lengths[0, :], bl)
        np.testing.assert_almost_equal(trj_single.unitcell_angles[0, :], al)
        ...
        trj_single, _ = replicate_polymer.replicate_pdb(f2, [1, 1, 1], boxangles=[88.0, 78.9, 90.0])
        bl = np.array([0.4123, 0.4686, 1.0974], dtype=np.float32)
        al = np.array([88.0, 78.9, 90.0], dtype=np.float32)
        np.testing.assert_almost_equal(trj_single.unitcell_lengths[0, :], bl)
        np.testing.assert_almost_equal(trj_single.unitcell_angles[0, :], al)
    ...
    def test_02_shift_positions(self):
        ...
        f1 = "./data/103191_noctane.order_nocryst.pdb"
        m = "\t\tPDB without CRYST section ({}): ".format(f1)
        trj_single, _ = replicate_polymer.replicate_pdb(f1, [1, 1, 1], boxlength=[0.4123, 0.4686, 1.0974],
