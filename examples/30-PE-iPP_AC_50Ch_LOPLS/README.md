...
The [file](./01-PREPARE/iPP-PE_40mon_25-25Ch_residues.pdb) is suitable to be typed using [replicate_polymer](https://github.com/jrdcasa/replicate_polymer_topology) software. If the program is applied directly, both PE and iPP topology files will be typed with L-OPLS type atoms, which is not valid for iPP polymer (see directory [99-DIRECT_NOVALID](./99-DIRECT_NOVALID/)). However, the [coordinate file](./99-DIRECT_NOVALID/iPP-PE_40mon_25-25Ch_residues_replicate.gro) is valid and it will be used in the next steps.
