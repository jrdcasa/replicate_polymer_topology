...
# =============================================================================================
def _check_independent_residues_cJ(molecule):
    """Check to see if residues will constitute independent graphs."""
    for res in molecule._universe.residues:
        atoms_in_residue = set([*res.atoms])
        bond_partners_in_residue = [
            item
            for sublist in [atom.bonded_atoms for atom in res.atoms]
            for item in sublist
        ]
...
# =============================================================================================
def _unwrap_typemap_cJ(structure, residue_map):
    master_typemap = {
...
# =============================================================================================
def _check_dihedrals(data, structure, non_openmm_potentials_terms, verbose,
                     assert_dihedral_params, assert_improper_params, logger=None):
    ...
    if data.propers and len(data.propers) != len(proper_dihedrals) + \
        ...
        if data.propers and len(data.propers) < len(proper_dihedrals) + len(structure.rb_torsions):
            ...
            list_idx_toxwaerd = [item[0] for item in non_openmm_potentials_terms['toxwaerd']['torsions']]
            ...
            for item in structure.rb_torsions:
                ...
                if tt in data.propers:
                    no_typed.remove(tt)
            for item in list_idx_toxwaerd:
                tt = (item[0], item[1], item[2], item[3])
                if tt in list_idx_toxwaerd:
                    no_typed.remove(tt)
...
class Forcefield(app.ForceField):
    ...
    def __init__(self, forcefield_files=None, name=None, validation=True, debug=False, logger=None):
        ...
        if validation:
            for ff_file_name in preprocessed_files:
                self.validator = Validator(ff_file_name, debug)
        ...
        if len(preprocessed_files) == 1:
            ...
            if len(set(self._combining_rule)) == 1:
                ...
                m1 = "\t\tERROR: Inconsistent combining_rule among loaded " \
                     "forcefield files. (forcefield.py: __init__).\n"
                m2 = "\t\tMolecule cannot be typed!!!!!. Exiting....\n"
                m = "\t\t" + len(m1) * "*" + "\n"
        ...
        self.parser = Smart.SMARTS(self.non_element_types)
        self._system_data = None
        try:
            self.all_forces_included_xml = self.validator.all_forces_included_xml
        except (UnboundLocalError, AttributeError):
            self.all_forces_included_xml = None
    ...
    def lj14scale(self):
        ...
        try:
            ...
            m1 = "\t\tERROR: Cannot get lj14Scale for the forcefield " \
                 "because it doesn't have NonBondedForce. (forcefield.py).\n"
            m2 = "\t\tMolecule cannot be typed!!!!!. Exiting....\n"
            m = "\t\t" + len(m1) * "*" + "\n"
            print("\n" + m + m1 + m2 + m) if self._logger is None else self._logger.info("\n" + m + m1 + m2 + m)
            exit()
    ...
    def coulomb14scale(self):
        ...
        try:
            ...
            m1 = "\t\tERROR: Cannot get coulomb14scale for the forcefield " \
                 "because it doesn't have NonBondedForce. (forcefield.py).\n"
            m2 = "\t\tMolecule cannot be typed!!!!!. Exiting....\n"
            m = "\t\t" + len(m1) * "*" + "\n"
            print("\n" + m + m1 + m2 + m) if self._logger is None else self._logger.info("\n" + m + m1 + m2 + m)
    ...
    def registerAtomType(self, parameters):
        ...
        if name in self._atomTypes:
            m1 = "\t\tERROR: Found multiple definitions for atom type: ".format(name)
            m2 = "\t\tMolecule cannot be typed!!!!!. Exiting....\n"
            m = "\t\t" + len(m1) * "*" + "\n"
            print("\n" + m + m1 + m2 + m) if self._logger is None else self._logger.info("\n" + m + m1 + m2 + m)
            exit()
    ...
    def apply(self, molecule, use_residue_map=True, assert_bond_params=True,
        ...
        typemap = self.run_atomtyping_cJ(molecule, use_residue_map=use_residue_map, logger=logger, **kwargs)

        self._apply_typemap_cJ(molecule, typemap, logger=logger)

        # openmm_impropers list contains the angles with structure:
        #                  (j) -- (i) --(k)
        #                          |
        #                         (l)
        # Be careful because the order jkl is arbitrary
    ...
    # =============================================================================================
    def run_atomtyping_cJ(self, molecule, use_residue_map=True, logger=None, **kwargs):
        ...
        if use_residue_map:
            ...
            if independent_residues:
                ...
                typemap = find_atomtypes(molecule._topology, forcefield=self, logger=logger)
    ...
    def parametrize_system(self, structure=None,
        ...
        system = self.createSystem(topology_openmm, *args, **kwargs)

        atoms_first_molecule_each_kind_dict = self._get_firstmolecule_eachkind(structure)

        _separate_urey_bradleys(system, topology_openmm)

        data = self._system_data

        structure_parmed = pmd.openmm.load_topology(topology=topology_openmm, system=system)

        non_openmm_potentials_terms["toxwaerd"], _ = self.check_toxwaerd_terms_cj(structure_parmed, data,
                                                                                  atoms_first_molecule_each_kind_dict)

        structure_parmed.bonds.sort(key=lambda x: x.atom1.idx)
        structure_parmed.positions = positions_openmm
        box_vectors = topology_openmm.getPeriodicBoxVectors()
        if box_vectors is not None:
            structure_parmed.box_vectors = box_vectors

        _check_bonds(data, structure_parmed, assert_bond_params, logger=logger)
        _check_angles(data, structure_parmed, verbose, assert_angle_params, logger=logger)
        # Include improper angles
        _check_dihedrals(data, structure_parmed, non_openmm_potentials_terms, verbose, assert_dihedral_params,
                         assert_improper_params, logger=logger)

        try:
            structure_parmed.combining_rule = self.combining_rule
        except ValueError as e:
            # Parmed raises ValueError: combining_rule must be 'lorentz' or 'geometric'
            if str(e).startswith("combining_rule must be"):
                m1 = "\t\tERROR: Combination rule {} is not implemented.\n".format(self.combining_rule)
                m2 = "\t\tMolecule cannot be typed!!!!!. Exiting....\n"
                m = "\t\t" + len(m1) * "*" + "\n"
                print("\n"+m+m1+m2+m) if self._logger is None else self._logger.info("\n"+m+m1+m2+m)
                exit()
        ...
        total_charge = sum([atom.charge for atom in structure_parmed.atoms])
        ...
        return structure_parmed, non_openmm_potentials_terms
    ...
    # =============================================================================================
    def _apply_typemap_cJ(self, structure, typemap, logger=None):
    ...
    #     return topology, positions
    #
    def _patch_parmed_adjusts(self, structure, combining_rule="geometric", logger=None):
        if combining_rule != "geometric":
            m1 = "\t\tERROR: UnimplementedCombinationRuleError.\n"
            m2 = "\t\tMolecule cannot be typed!!!!!. Exiting....\n"
            m = "\t\t" + len(m1) * "*" + "\n"
            print("\n"+m+m1+m2+m) if logger is None else logger.info("\n"+m+m1+m2+m)
            exit()
    ...
    # =========================================================================
    def check_toxwaerd_terms_cj(self, structure, data, first_molecule_each_kind_dict):

        """
        Check if there is Toxwaerd terms in the XML file

        Args:
            structure: Parmed structure
            data: SystemData from OpenMM
        ...
        toxwaerd_onlyonekind = defaultdict(list)
        ...
        for child in leaf:
            class1 = child.attrib['class1']
            class2 = child.attrib['class2']
            class3 = child.attrib['class3']
            class4 = child.attrib['class4']
            coeff = [child.attrib["k0"], child.attrib["k1"], child.attrib["k2"],
                     child.attrib["k3"], child.attrib["k4"], child.attrib["k5"],
                     child.attrib["k6"], child.attrib["k7"], child.attrib["k8"]]
            periodicity = [child.attrib["periodicity0"], child.attrib["periodicity1"],
                           child.attrib["periodicity2"], child.attrib["periodicity3"],
                           child.attrib["periodicity4"], child.attrib["periodicity5"],
                           child.attrib["periodicity6"], child.attrib["periodicity7"],
                           child.attrib["periodicity8"]]
            phase = [child.attrib["phase0"], child.attrib["phase1"],
                     child.attrib["phase2"], child.attrib["phase3"],
                     child.attrib["phase4"], child.attrib["phase5"],
                     child.attrib["phase6"], child.attrib["phase7"],
                     child.attrib["phase8"]]
        ...
        for iproper in data.propers:
            ...
            for ikind, ivalues in first_molecule_each_kind_dict.items():
                if at1 in ivalues:
                    toxwaerd_onlyonekind[ikind].append([(at1, at2, at3, at4), str_f])
        toxwaerd_dict["torsions"] = toxwaerd_torsions
        toxwaerd_dict["torsiononlyonekind"] = toxwaerd_onlyonekind
        toxwaerd_dict["atoms_first_molecule_each_kind_dict"] = first_molecule_each_kind_dict

        return toxwaerd_dict, toxwaerd_torsions

    # =============================================================================================
    def _get_firstmolecule_eachkind(self, structure_from_pdb):

        atoms_first_molecule_each_kind_dict = defaultdict()


        # Number kinds of molecules
        nkinds = len(dict((v, k) for k, v in structure_from_pdb._atom3d_kindmolecule.items()))

        # idx_for_first_molecule_each_type
        for imol in structure_from_pdb._topology._nmols:
            fat = imol[0]
            ikind = structure_from_pdb._atom3d_kindmolecule[fat]
            if ikind not in atoms_first_molecule_each_kind_dict:
                atoms_first_molecule_each_kind_dict[ikind] = imol


        return atoms_first_molecule_each_kind_dict
